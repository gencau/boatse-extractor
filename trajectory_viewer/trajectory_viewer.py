...
def _make_label(ev: dict) -> str:
    ...
    if t == "end":
        reprompts = ev.get("reprompts", 0)
        return f"Pipeline complete — reprompts: {reprompts}"
    if t == "summary":
    ...
    if t == "bm25_query":
        preview = ev.get("query_preview", "")[:60]
        return f"BM25 query — {preview}…"
    if t == "parse":
        ok = ev.get("ok", False)
        return f"Parse {'✓' if ok else '✗'} — {step}"
    if t == "reprompt":
        attempt = ev.get("attempt", "?")
        return f"Re-prompt (attempt {attempt}) — format failure"
    if t == "reprompt_success":
        files = ev.get("ranked_files", [])
        return f"Re-prompt succeeded — {len(files)} files returned"
    if t in ("self_eval", "self_eval_applied", "final_answer"):
        files = ev.get("ranked_files", [])
        label = {"self_eval": "Self-eval prompt", "self_eval_applied": "Self-eval applied", "final_answer": "Final answer"}.get(t, t)
        return f"{label} — top file: {files[0].split('/')[-1] if files else '?'}"
    if t == "preview_filter":
...
def _fmt_ts(ts_str: str) -> str:
    """Convert ISO timestamp → HH:MM:SS."""
    try:
...
def _body_rows(ev: dict) -> list[tuple[str, str]]:
    """Return (key, value) pairs for the expanded body, skipping noisy fields."""
...
(function(){
  ...
  viewer.querySelector('.traj-controls').addEventListener('click', function(e){
    ...
    viewer.querySelectorAll('.traj-btn').forEach(function(b){ b.classList.toggle('active', b===btn); });
    viewer.querySelectorAll('.traj-event, .traj-divider').forEach(function(el){
      if (el.classList.contains('traj-divider')) {
        el.style.display = '';
        return;
      }
...
def _build_html(events: list[dict]) -> str:
    ...
    for ev in events:
        ...
        if ranked and isinstance(ranked, list) and isinstance(ranked[0], str):
        ...
        if ev.get("type") == "tool_result" and ev.get("view_file"):
            ...
            body_parts.append(f'<div class="traj-kv"><span class="traj-kv-k">file</span>'
                               f'<span class="traj-kv-v">{_esc(ev["view_file"])}</span></div>')
            body_parts.append(f'<div class="traj-code">{_esc(content[:1200])}{"..." if len(content)>1200 else ""}</div>')
        ...
        for k, v in ev.items():
            ...
            if "\n" in val or len(val) > 100:
                body_parts.append(f'<div class="traj-kv"><span class="traj-kv-k">{_esc(k)}</span></div>')
                body_parts.append(f'<div class="traj-code">{_esc(val[:1500])}{"..." if len(val)>1500 else ""}</div>')
            else:
                body_parts.append(f'<div class="traj-kv"><span class="traj-kv-k">{_esc(k)}</span>'
                                   f'<span class="traj-kv-v">{_esc(val)}</span></div>')

        # messages array (prompts) — show abbreviated
        if ev.get("messages"):
            for msg in ev["messages"]:
                ...
                body_parts.append(
                    f'<div class="traj-kv"><span class="traj-kv-k">[{_esc(role)}]</span></div>'
                    f'<div class="traj-code">{_esc(preview)}</div>'
                )
    ...
    return (
        ...
        f'  <div class="traj-timeline">{timeline_html}</div>'
        f'</div>' +
        _JS
    )
...
def render_trajectory(source=None, *, events=None, height: int = 700):
    ...
    # Wrap in a scrollable iframe-like div for Colab
    wrapped = f"""
